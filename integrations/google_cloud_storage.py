...
the_bucket = storage.bucket(name='audioland-dub.appspot.com')
...
def upload_blob(bucket, source_file_name, destination_blob_name):
    ...
    blob = the_bucket.blob(destination_blob_name)
...
download_blob(the_bucket, source_blob_name, destination_file_name)

# здесь отрабатывает нейронка
# video path = destination_file_name

# Хардкод для тестов (загрузили обработанное)
...
upload_blob(the_bucket, source_file_name, destination_blob_name)
